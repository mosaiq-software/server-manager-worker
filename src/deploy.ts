...
const injectCompose = async (projectId: string, compose: string, logId: string): Promise<void> => {
    if (process.env.PRODUCTION !== 'true') {
        console.log('Not in production mode, skipping compose injection');
        await sendLogToControlPlane(logId, 'Not in production mode, skipping compose injection\n', DeploymentState.DEPLOYING);
        return;
    }
    ...
    for (const file of composeFiles) {
        try {
            const path = `${process.env.DEPLOYMENT_PATH}/${projectId}/${file}`;
            await fs.rm(path, { force: true, recursive: true });
            await sendLogToControlPlane(logId, `Removed old compose file: ${path}\n`, DeploymentState.DEPLOYING);
        } catch (e: any) {
            // ignore errors
        }
    }
    ...
    try {
        const composeWithHeader = `# Generated by NSM - Do not edit directly\n${compose}`;
        await fs.writeFile(`${process.env.DEPLOYMENT_PATH}/${projectId}/${composeFiles[0]}`, composeWithHeader);
        await sendLogToControlPlane(logId, 'Successfully injected compose file\n', DeploymentState.DEPLOYING);
    } catch (e: any) {
        throw new Error(`Error injecting dotenv file: ${e.message}`);
    }
};
